        ...
        <div class="row">
            ...
            <div class="col-md-4 col-xs-12 order-md-last ml-2 ml-md-auto order-first pl-0">
                <ul class="faq-list">
                    ...
                    <li><a @if($faq->id == $f->id) class="color-C11800" @endif href="{{url('navigator/faq', ($f->id>1)?$f->id:'1')}}"> <img @if($faq->id == $f->id) src="{{asset('img/arrow-dots-red.svg')}}" @else src="{{asset('img/arrow-dots-black.svg')}}" @endif >{{$f->question}}</a></li>
                        @endforeach
                </ul>
            </div>
        </div>
