        ...
        <div class="row">
                ...
                <table class="table table-striped table-view">
                    ...
                    @for($i = 0; $i < count($us); $i++)
                        ...
                        <tr>
                            ...
                            <td>
                            @if($us[$i]->description)
                                <a class="college-list-a" href="{{route('college.view', ['name' => ($type != 2)?'colleges':'universities', 'id' => $us[$i]->university_id])}}">{{$us[$i]->name_ru}}</a>
                            @else
            ...
            <div class="col-md-4 pl-0 order-md-last ml-2 ml-md-auto order-first">
                ...
                @foreach(\App\Profile::where('forCollege', 2-$type)->get() as $p)
                        <li><a href="{{url('qazaqstan/navigator/rating', [($type-1)?'universities':'colleges', $type, $p->id])}}" @if(($class ?? '') == $type.$p->id) class="color-C11800" @endif > @if(($class ?? '') == $type.$p->id) <img src="{{asset('img/arrow-dots-red.svg')}}" alt=""> @else <img src="{{asset('img/arrow-dots-black.svg')}}" alt=""> @endif {{$p->name}} </a></li>
                    @endforeach
                </ul>
            </div>
        </div>
