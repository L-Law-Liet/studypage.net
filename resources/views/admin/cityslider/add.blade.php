                ...
                <div class="box-body">
                    <form action="{{ $action }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                <input required type="text" name="name_ru" class="form-control" @if(is_object($cityslider)) value="{{ $cityslider->name_ru }}" @endif>
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="col-md-3">Заголовок</label>
                            <div class="col-md-9">
                                <input required type="text" name="announce" class="form-control" @if(is_object($cityslider)) value="{{ $cityslider->announce }}" @endif>
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="col-md-3">Описание</label>
                            <div class="col-md-9">
                                <textarea required name="description" class="form-control">@if(is_object($cityslider)) {{ $cityslider->description }} @endif</textarea>
                            </div>
                            </div>
                            <div class="form-group row">
                                <label class="col-md-3">Изображение</label>
                                <div class="col-md-9">
                                    <input {{isset($cityslider->image)?'':'required'}} type="file" name="image">
                                </div>
                            </div>
                        <div class="form-group row">
