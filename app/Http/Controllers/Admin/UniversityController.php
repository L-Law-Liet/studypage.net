...
class UniversityController
{
    ...
    public function postAdd($id = null){
        ...
        if (!empty($_COOKIE['page'])) {
            ...
            return \redirect((session()->get('n'))?'admin/university?n='.session()->get('n'):'admin/university')->with('success', 'Запись успешна сохранена');
        }
    }
    ...
    public function getDelete($id){
        ...
        $costs = CostEducation::where('university_id', $university->id)->get();
        if (count($costs)>0){
        ...
        if (!empty($_COOKIE['page'])) {
            ...
            return \redirect((session()->get('n'))?'admin/university?n='.session()->get('n'):'admin/university')->with('errors', 'Запись успешна удалена');
        }
    }
...
//                $file->move(public_path() . '/img/universities/', $fileName);
//                $u->image = $data['image'];
//                $u->logo = 1;
//            }
//        }
...
//        $u->dormitory = $data['dormitory'];
//        $u->military_dep = $data['military_dep'];
//        $u->web_site = $data['web_site'];
        $u->save();
        if (!empty($_COOKIE['page'])) {
            return \redirect('admin/list?page='.$_COOKIE['page'])->with('success', 'Запись успешна сохранена');
        } else {
            return \redirect((session()->get('n'))?'admin/list?n='.session()->get('n'):'admin/list')->with('success', 'Запись успешна сохранена');
        }
    }
...
//        $university->web_site = null;
        ...
        if (!empty($_COOKIE['page'])) {
            ...
            return \redirect((session()->get('n'))?'admin/list?n='.session()->get('n'):'admin/list')->with('errors', 'Запись успешна удалена');
        }
    }
